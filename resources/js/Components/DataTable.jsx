...
export function DataTable({ data, action, tbStructure, onNew, onUpdate, onDelete , onSee }) {
    ...
    const searchData = (searchValue) => {
        setCurrentPage(1)
        const searchedData = data.filter(item => item.name.toLocaleLowerCase().includes(searchValue))
                                        ...
                                        <td className="px-5 py-3 border-b border-gray-200 bg-white text-sm">
                                            <div className='flex gap-4'>
                                                ...
                                                {onSee &&
                                                    ...
                                                    <span className='cursor-pointer bg-green-400 rounded-md px-2 text-white font-bold hover:bg-green-700' onClick={() => { onSee(val.id) }}>
                                                        ver
                                                    </span>
                                                }
